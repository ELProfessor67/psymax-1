...
export const handleCreateTransport = async (direction: 'send' | 'recv', transportOptions: any) => {
    try {
        const transport = await webrtcService.createTransport(direction,transportOptions);
        return transport;
    } catch (error) {
...
export const handleConsumeTrack = async (consumer: any, remoteStream: MediaStream) => {
    try {
        const consumer = await webrtcService.consumeMedia(consumers, remoteStream);
        return consumer;
    } catch (error) {
...
export const handleProduceTrack= async (producer: any, localStream: MediaStream, kind: 'audio' | 'video') => {
    try {
        const consumer = await webrtcService.consumeMedia(producer,localStream);
        return consumer;
    } catch (error) {
        console.error('Error consuming track:', error);
        throw error;
...
// This custom hook is used to manage multiple states to keep the main page clean and organized.
// Instead of cluttering the main page with too many useState calls, we handle all related state logic here.
